<template>
  <div class="voucherEasytoBuy width height bg-f6 overflow-s" ref="vouchers"><!-- style="position: absolute"-->
    <!--有券金额状态-->
    <div class="height" v-if="voucherAmountNumber">
      <div class="vouchersInformation width pst-rlt" ref="voucherInformation" :class="Recharge==true?lHeight:sHeight">
        <div class="informationWarp">
          <div class="font-167 pst-rlt font-A9CFF8 mG-top">
            消费券总额度（元）
            <div class="SZDetailed cont pst-abs font-1" @click="toBudgetPage">收支明细</div><!--@click="tovoucherRechargePage"-->
            <div class="fr font-col-b" v-if="rechargeED>0" style="margin-top: 4rem"><span style="margin-left: 1rem">待圈存：{{rechargeED|moneyFormat}}元</span></div>
          </div>
    ...
    <div class="bg-f6 width" v-if="voucherAmountNull">
      <div class="fillIn-comter1 vouchers-conter pst-abs pos-top width bg-ff" style="height: 20rem;">
        ...
        <p class="cont font-a39">您尚未在易得多购买消费券</p>
...
<script>
  ...
  import {rechargePage} from '@/api/user/userLogin'
  ...
  export default {
    ...
    methods:{
      ...
      getfindTransferCouponList(canTransfer){
        ...
        let self = this;
        setTimeout(function () {
          self.sumAmount = self.$root.$data.queryvoucherData.yideduo.sumAmount;
          ...
          self.canTransferAmount = self.$root.$data.queryvoucherData.yideduo.canTransferAmount;
          self.noTransferAmount = self.$root.$data.queryvoucherData.yideduo.nonTransferAmount;
        },300)
      },
...
<style type="text/css">
  ...
  /*转让弹窗*/
  .voucherEasytoBuy .goTransfor{
    justify-content: center;
    display: flex;
  }
  .voucherEasytoBuy .goTransfor .zzClose{
    font-size:1.8rem;
    right:1.3333rem;
    top:0.5rem;
  }
  .voucherEasytoBuy .zzWrap{
    width:22.5rem;
    height:24.83333rem;
    border-radius:7px;
    background:#fff;
  ...
  .voucherEasytoBuy .zzWrap .zzTips1{
    margin-top:4rem;
  }
  .voucherEasytoBuy .zzWrap .zzTips2{
    color:#7E7A7A;
    margin-top:0.6666rem;
  }
  .voucherEasytoBuy .zzWrap img{
    margin-left: 50%;
    margin-top:1.5rem;
  ...
  /*新添加头部*/
  .voucherEasytoBuy .largeHeight{
    height:30%;
  }
  .voucherEasytoBuy .smallHeight{
    height:16.25rem;
  }
  .voucherEasytoBuy .vouchersInformation{
    background:#3B92EF;
  }
  .voucherEasytoBuy .vouchersInformation .informationWarp{
    /*width:95%;*/
    height:16.083333rem;
    margin:0 1.08333333rem;
    padding-top:1.3333333rem;
  }
  .voucherEasytoBuy .informationWarp .SZDetailed{
    background: #FFFFFF;
  ...
  .voucherEasytoBuy .informationWarp .useED{
    margin-top:0.3rem;
    font-size:2.583333rem;
  }
  .voucherEasytoBuy .informationWarp .stayRecharge{
    margin-top:0.58rem;
  }
  .voucherEasytoBuy .informationWarp .stayRecharge p:nth-of-type(2){
    width:3.6rem;
    height:1.3333rem;
    line-height:1.2rem;
    background:#fff;
  ...
  .voucherEasytoBuy .fixedArea{
    width:93%;
    height:5rem;
    bottom:0;
    z-index:555;
  }
  .voucherEasytoBuy .fixedArea .action{
    background:#fff;
    color:#2E2A2A;
  }
  .voucherEasytoBuy .fixedArea .normal{
    background:#B7DBFF;
    color:#3B92EF;
  }
  .voucherEasytoBuy .fixedArea .canTransfer,.canExpense{
    width:50%;
    height:100%;
  }
  .voucherEasytoBuy .fixedArea p:first-of-type{
    margin-top:0.6666667rem;
  }
  /*修改样式bug*/
  .voucherEasytoBuy .industry-border{
    border-bottom:solid 1rem #F6F6F6;
  }
  .voucherEasytoBuy .vouchersMarket{
    width:8.3333rem;
    height:2.083333rem;
  ...
  .voucherEasytoBuy .img_Bx1{
    width: 13rem;
    height: 13rem;
    margin-left: 8rem;
    padding-top: 50%;
  }
  .voucherEasytoBuy .fillIn-comter2{
    margin-top: 54.5%;
  }
  .voucherEasytoBuy .fillIn-comter2 .img_Bx{
    width: 13rem;
    height: 13rem;
    margin-left:30%;
    padding-top: 22%;
  }
  .voucherEasytoBuy .fillIn-comter1 .img_Bx{
    width: 13rem;
    height: 13rem;
    margin-left:30%;
    padding-top: 1rem;
  }
  .voucherEasytoBuy .img_line{
    width: 1px;
    height: 2.083333rem;
  }
  .voucherEasytoBuy .fillIn-comter2 .font-ipt{
    margin-left: 0;
  }
  /*头部*/
  .voucherEasytoBuy .vouchers-title{
    /*height: 6%;*/
    position: sticky;
    /*position:fixed;*/
    top:0;
  ...
  .voucherEasytoBuy .vouchers-title .v_title_img{
    width: 50%;
    /*line-height: 3.2rem;*/
    height:100%;
  }
  .voucherEasytoBuy .vouchers-title .v_title_img img{
    width: 1.4166666667rem;
    padding-left: 35%;
    padding-top: 0.8rem;
    float: left;
  }
  .voucherEasytoBuy .vouchers-title .v_title_img span{
    padding-left: 0.5rem;
    font-size: 1rem;
  ...
  /*券部分*/
  .voucherEasytoBuy .vouchers-container{
    float:left;
    overflow-y: scroll;
  }
  .voucherEasytoBuy .vouchers-footOp{
    bottom: 0;
    position: absolute;
    height: 3.666667rem;
    line-height: 3.6666667rem;
  }
  .voucherEasytoBuy .vouchers-conter .vouchers_Q{
    margin: 0 0.916rem;
    position: relative;
  }
  .voucherEasytoBuy .vouchers-conter .vouchers_Q_p{
    line-height: 4rem;
    margin: 0 0.65rem;
    font-size: 1.2rem;
  }
  .voucherEasytoBuy .vouchers-conter .Travel-farst{             /****************考虑删除**************************/
    width:96%;
    height:100%;
    position: relative;
    margin: 0 1rem;
  }
  .voucherEasytoBuy .vouchers-conter .Travel-farst .Travel-title{
    height: 2.083333rem;
    padding-top: 0.75rem;
    margin:0 1rem;
  }
  .voucherEasytoBuy .vouchers-conter .Travel-conter1{
  ...
  /*钱*/
  .voucherEasytoBuy .Travel-title .Travel-title-r{
    margin-right: 1.025rem;
    max-width: 48%;
  }
  .voucherEasytoBuy .Travel-title .Travel-title-r .money_1{
    font-size: 1.7rem;
  }
  .voucherEasytoBuy .Travel-title .Travel-title-r .money_2{
    font-size: 2.25rem;
    font-weight: bold;
  }
  .voucherEasytoBuy .Travel-title .Travel-title-r .money_3{
  ...
  .voucherEasytoBuy .v_screen .pst-fixed{
    margin: 6rem 0;
    width: 7rem;
  }
  .voucherEasytoBuy .v_screen .v_screen_mask{
    position: absolute;
    font-size: 1.5rem;
    width: 89%;
    margin-left: 11%;
  ...
  .voucherEasytoBuy .vouchers-conter .vouchers-title{
    margin-top: 0;
    position: absolute;
    top: 0.3rem;
    z-index: 1000;
  ...
  .voucherEasytoBuy .Travel-farst .Travel-title .Travel-title-l{
    max-width: 50%;
  }
  .wrapcenter_tips{
    width: 22.5rem;
  ...
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon{
    border-radius: 0.333333rem;
    opacity:0.96;
    margin: 0.5rem 0;
  }
  .voucherEasytoBuy .Market-Coupon-list{
    margin: 0.5rem 0.83333333rem;
    width: 94.5%;
  }
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .Market-Coupon-l{
    margin: 1.666666667rem 1rem;
    width: 9rem;
    height: 6.16667rem;
    display: -webkit-box;
  }
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .Market-Coupon-l .Market-Coupon-img{
    -webkit-border-radius: 0.3333333rem;
    -moz-border-radius: 0.3333333rem;
    border-radius: 0.3333333rem;
  }
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .bt-color-M-btn{
    margin: 3rem 0.82rem;
    border-radius: 3rem;
    line-height: 2.3rem;
    width: 5rem;
  ...
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .Market-C-cont{
    padding: 1.166666666rem 0;
    width:37%;
  }
  .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .Market-C-cont .ellipsis_name_2{
    overflow: hidden;
    text-overflow: ellipsis;
    display: -webkit-box;
  ...
  @media screen and (max-width:321px){
    ...
    .voucherEasytoBuy .vouchers-conter .Travel-farst {
      margin: 0 .7rem;
    }
    .Travel-farst .Travel-sec {
    ...
    .voucherEasytoBuy .v_screen_mask .mask_Popup .mask_Popup_type_check {
      background:url("../../../../assets/img/Coupon/icon_label@2x.png") no-repeat 80% 48%;
      background-size: 10rem;
    }
    .voucherEasytoBuy .Travel-farst .Travel-sec .Travel-sec-l {
      max-width: 16rem;
    }
    .voucherEasytoBuy .Market-Coupon-list .Market-Coupon .Market-Coupon-l{
      width: 7rem;
  ...
  @media screen and (min-width:760px)and (max-width: 1024px){
    .voucherEasytoBuy .industry_btn_l div{
      /*padding-right:0.7rem;*/
    }
    .voucherEasytoBuy.vouchers-conterIpad{
      height: 89.1%
    }
    .voucherEasytoBuy .fillIn-comter2 .img_Bx{
      margin-left:30%;
